...
try:#捕捉所有错误（如果被弹出）
    class FlagSet:#指令集
         def __getattr__(self, name):
            setattr(self, name, name)#设置值
            return name

    class ImageButton:#图片按钮动画
        ...
        def button_ani(self, time):
            ...
            self.spend = self.frame / time#V=s/t
    ...
    class DrawCenter:#定义在屏幕中间绘制的类
        @staticmethod#声明静态方法
        def draw_rect(Surface, color, size, width = 0, horn = 90):
            ...
            return rect_xy#输出绘制位置

        @staticmethod#声明静态方法
        def make_rect(size):
            global w_face, h_face
            rect_xy = ((w_face - size[0]) // 2, (h_face - size[1]) // 2)#输出绘制位置

            return pygame.Rect(rect_xy, size)#输出绘制位置

    class ImageHandle:
        def __init__(self, file_image_name, re_image_name=None, type=None):#类构造函数
            self.img = Image.open(file_image_name)#读取文件

            if re_image_name is not None:#判定转换后文件名是否传入
                self.re_image_name = re_image_name#读取转换后文件名
            else:
                self.re_image_name = file_image_name#设为原文件名

            if type is not None: #判定格式是否传入
                self.type = type#读取保存格式
            else:
                self.type = file_image_name.split('.')[1]#设为文件默认格式
        ...
        def gaussian_blur(self, fuzzy_kernel):
            self.re_img = self.img.filter(ImageFilter.GaussianBlur(radius=fuzzy_kernel))#做高斯模糊处理
            self.re_img.save(self.re_image_name, self.type)#保存图片

    def warring_frame(home, warring_image, return_xy=False):#创建提示框架
        warring_region = DrawCenter.make_rect((w_face // 2, h_face // 2))#创建提示窗口框架矩形范围

        warring = home.subsurface(warring_region)
        pygame.image.save(warring, warring_image)#以提示窗口框架矩形范围在屏幕上截取并保存

        warring_handle = ImageHandle(warring_image, warring_image)
        warring_handle.gaussian_blur(32)#把刚刚的图片更高级高斯模糊
        ...
        return warring if return_xy else warring, (warring_region.x, warring_region.y)#返回绘制好的Surface
    ...
    def windows_home_cut():#截取当前桌面
        ...
        img = ImageHandle(windows_home_image)#获取截取图片
        img.gaussian_blur(16)#做高斯模糊处理
    ...
    try:#尝试导入素材文件
        ...
        for i in range(0, 25):#更改原退出按钮素材分辨率
            ...
            exit_image_handle.resize_image(
                         w_face // 44 + i, w_face // 44 + i)#创建从小到大的退出图片序列，动画用
        ...
        str_error = ''
        for i in str(error):#使用字符串方法把错误的字符连接起来
            str_error = str_error + str(hex(i))#转化为16进制字符串
        ...
        home_word_1_example = WordButton(home_word_1, home_word_1_black, home_word_1_xy)
        home_word_2_example = WordButton(home_word_2, home_word_2_black, home_word_2_xy)
        home_word_3_example = WordButton(home_word_3, home_word_3_black, home_word_3_xy)
        #创建主页文字按钮动画实例

        home_exit_example = ImageButton(micro_exit_list, home_exit_xy)#创建图片按钮动画实例

        warring_frame_draw = False
        #初始化变量
        home_flag_set = FlagSet()#初始化指令集
        ...
        while True:#多级用户界面控制
            while True:#主循环
                ...
                if home_exit_example.get_button_collide(type_mouse_xy):#判定是否开始绘制提示
                    home_interface = home_flag_set.WARRINGDRAW#指示开始绘制
                    print('nb')
                    break#进入子页面
                ...
                home_exit = home_exit_example.button_ani(0.075)
                #用动画类更新Surface
                ...
                '''
            print('nb')
            if home_interface == home_flag_set.WARRINGDRAW:#提示信息是否开始绘制
                home_exit_warring, home_exit_warring_xy = warring_frame(
                    windows_home, home_exit_warring_image, return_xy=True)#创建提示框架

                home_exit_warring.blit(home_word_surface_list[0], (0, 0))#绘制提示文字
                '''
                没做完
                '''
                while True:#退出提示窗口
                    ...
                    home.blit(home_exit_warring, home_exit_warring_xy)#绘制提示
